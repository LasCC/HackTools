...
export const initializeChecklist = (checklists: any) => {
  return checklists.map((test) => ({
    ...test,
    testCaseStatus: Object.values(TestCaseStatus).includes(
  ...
  create<any>(
    persist(
      (set, get) => ({
        ...
        handleCSVExport: () => {
          const data = get().stateFlattenedChecklists.map(
            ({ id, description, reference, testCaseStatus, observations }) => ({
              id,
              description,
              reference,
              testCaseStatus,
          ...
          saveAs(
            blob,
            `htool_${new Date().toISOString()}_${new Date().getTime()}.csv`
          );
        },
