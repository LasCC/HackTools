...
const GraphComponent = () => {
  ...
  if (data.length === 0 || !activeScanResult)
    return (
                  ...
                  <List.Item>
                    <List.Item.Meta
                      title={
                        <Text
                          strong
                        >{`${service.service}:${service.port}/tcp`}</Text>
                      }
  ...
  return (
    <>
      <Row key={"graph_canvas"} gutter={[16, 16]}>
        <Col
          key={"graph_canvas"}
          style={{
      ...
      <Drawer
        title={
          currentNode
            ? currentNode.type === "host"
              ? currentNode.address
              : `${currentNode.service}:${currentNode.port}`
            : "N/A"
        }
        placement="right"
        closable={true}
