...
const LayoutChoice = () => {
	...
	const modes: Array<{
	...
	}> = [
		{
			...
			title: "Web Security",
			description:
				"Various payloads and tools related to web security assessment.",
			shortcut: (
		...
		},
		{
			...
			description:
				"Reconnaissance, exploitation and post-exploitation, OS related tools.",
			shortcut: (
				<div style={{ marginTop: 5 }}>
					<Text keyboard>CTRL</Text>
					<Text keyboard>ALT</Text>
		...
		},
		{
			...
			description:
				"Android and iOS related payloads for debugging and app instrumentation.",
			shortcut: (
				<div style={{ marginTop: 5 }}>
					<Text keyboard>CTRL</Text>
					<Text keyboard>ALT</Text>
		...
		},
		{
			...
			description:
				"Various tools such as a private cheatsheets, advanced methodology, etc.",
			shortcut: (
				<div style={{ marginTop: 5 }}>
					<Text keyboard>CTRL</Text>
					<Text keyboard>ALT</Text>
	...
	const mappedModules = modes.map((mode) => (
		<Col key={mode.name} span={12} xxl={6}>
			<Card
				style={{
					width: "100%",
					marginTop: 16,
			...
			>
				<Meta title={mode.title} description={mode.description} />
				<Meta description={mode.shortcut} />
			</Card>
		</Col>
	));
	...
	return (
		<>
			<Row gutter={[24, 16]}>{mappedModules}</Row>
			<div style={{ marginTop: 8 }}>
				<Alert
					message="New keyboard shortcuts"
					description={
						<span>
							You can now use the keyboard shortcut to switch between the
							different modes, or use your <Text keyboard>{keySymbol}</Text> +{" "}
							<Text keyboard>k</Text> to open the command palette.
						</span>
					}
					type="info"
					showIcon
