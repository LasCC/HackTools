...
const DataEncoding = () => {
  ...
  const handleClick = (type: string) => {
        ...
        switch (encMode) {
          ...
          case "hex":
            try {
              ...
              errorMessage =
                "Incorrect hexadecimal, please try something else.";
            }
            break;
        }
        break;
    ...
    setOutput(
      errorMessage ? "Unable to decode properly: " + errorMessage : output
    );
    if (errorMessage) {
  ...
  const handleEncModeList = (type: {
    key: React.SetStateAction<string | any>;
  }) => {
  ...
  return (
    <div>
      <div style={{ margin: 15 }}>
        ...
        <Paragraph>
          Adversaries may encode data to make the content of command and control
          traffic more difficult to detect. Command and control (C2) information
          can be encoded using a standard data encoding system. Use of data
          encoding may adhere to existing protocol specifications and includes
          use of ASCII, Unicode, Base64, MIME, or other binary-to-text and
          character encoding systems.Some data encoding systems may also result
          in data compression, such as gzip.
        </Paragraph>
      </div>
      <Divider dashed />
      <div
        ...
        <div style={{ display: "flex", alignItems: "center" }}>
          <Dropdown overlay={menu}>
            <a className="ant-dropdown-link">
              {encMode} <DownOutlined style={{ padding: 10, marginTop: 5 }} />
            </a>
          </Dropdown>
          ...
          >
            <BiLockOpenAlt style={{ marginRight: 2 }} />
            Decode
          </Button>
          <Button
            type="text"
