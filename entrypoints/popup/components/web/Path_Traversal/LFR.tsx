...
export default function LFI() {
	...
	const phpWrapperFiltersManual = [
		{
			name: "String Filters",
			url: "https://www.php.net/manual/en/filters.string.php",
		},
		{
			name: "Conversion Filters",
	...
	return (
		<div style={{ margin: 15 }}>
			...
			<Paragraph>
				LFR (Local File Read) is a vulnerability that allows an attacker to read
				files on the server running the web application. LFI is similar to the
				LFR vulnerability, but it allows an attacker to <b>include</b> a file on
				the web server which is evaluated interpreter, thus allowing code
				execution.
			</Paragraph>
			<Paragraph>
				Typically this is exploited because of a lack of input validation. The
				attacker can use a relative path to read files on the server.
			</Paragraph>
			<div key="a">
				<Title level={3}>Directory traversal</Title>
				<Paragraph ellipsis={true}>
					<pre>
						<Text copyable>{directoryTraversal}</Text>
					</pre>
				</Paragraph>
			...
			<div key="b">
				...
				<Paragraph ellipsis={true}>
					<pre>
						<Text copyable>{phpWrapperExpect}</Text>
					</pre>
				</Paragraph>
			...
			<div key="c">
				...
				<Paragraph>You can find other filters in PHP manual:</Paragraph>
				<ul>
				...
				<Paragraph ellipsis={true}>
					<pre>
						<Text copyable>{phpWrapperFilter}</Text>
					</pre>
				</Paragraph>
			...
			<div key="d">
				...
				<Paragraph>
					...
					attacker to execute arbitrary PHP code by including a remote file
					located on a server controlled by the attacker.
				</Paragraph>
				<Paragraph ellipsis={true}>
					<pre>
						<Text copyable>{phpRfi}</Text>
					</pre>
				</Paragraph>
				{copyButton(phpRfi)}
			</div>
...
function mapFileEntries(
	arr: { title: string }[]
): ReactElement<typeof Paragraph>[] {
	return arr.map((k, i) => {
		return (
			<Paragraph key={i}>
				<pre>
					<Text copyable>{k.title}</Text>
				</pre>
			</Paragraph>
		);
	});
