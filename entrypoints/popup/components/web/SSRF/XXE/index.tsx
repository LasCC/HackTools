...
const index = () => {
	...
	const columns: ColumnsType<DataType> = [
		...
		},
		{
			...
			key: "document_type",
			filters: Array.from(
				new Set(payloadsData.map((item) => item.document_type.join(", ")))
			).map((document_type) => ({ text: document_type, value: document_type })),
			onFilter: (value, record) =>
				record.document_type.indexOf(String(value)) === 0,
			render: (text, doc) => renderTag(text),
		},
		{
			title: "Transmission",
			dataIndex: "oracle",
			key: "oracle",
			render: (text, primitive) => <>{renderTag(primitive.oracle)}</>,
		},
		{
			title: "Primitive",
			dataIndex: "primitive",
			key: "primitive",
			filters: Array.from(
				new Set(payloadsData.map((item) => item.primitive.join(", ")))
			...
			render: (_, { payload }) => (
				<>
					<Dropdown.Button
							...
							onClick: (e) => {
								switch (e.key) {
									...
									case "2":
										...
										info();
										const encodedRessourceName =
											"data://text/plain;base64," + btoa(XXEfilename);
										navigator.clipboard.writeText(
											payload[0].main.replace(
												/\{RESSOURCE\}/g,
												encodedRessourceName
											)
										);
										break;
									case "3":
										...
										info();
										navigator.clipboard.writeText(
											encodeURIComponent(encodeURIComponent(payload[0].main))
										);
										break;
									case "4":
										// double url encoded
						...
						onClick={() => {
							info();
							navigator.clipboard.writeText(payload[0].main);
						}}
					>
						Copy
					</Dropdown.Button>
	...
	const filteredDatabyCurrentXXEType = data.filter((item) =>
		XXEtype === XXETypes.INBAND
			? item.oracle.includes("INBAND")
			: item.oracle.includes("OOB")
	);

	const formatPayload = (payload) =>
		payload
			.replace(/\{RESSOURCE\}/g, XXEfilename)
			.replace(/\{DTD_PATH\}/g, XXEDTDPATH)
			.replace(/\{REMOTE_SERVER\}/g, REMOTEXXEServer);
			...
			<Row gutter={[16, 16]}>
				<Col span={24}>
					<Table
						columns={columns}
						locale={{
							emptyText: `No payloads found for "${XXEtype}" XXE type. Try adjusting your search criteria or select a different type.`,
						}}
						expandable={{
							expandedRowRender: (record) => (
								<>
									{record.payload.map((pld, id) => (
										<>
											<Paragraph key={id}>
												{formatPayload(pld.description)}
											</Paragraph>
											<Text code copyable editable key={id}>
												{formatPayload(pld.main)}
											</Text>
											<Divider />
										</>
									))}
