...
export default function LayoutApp(props: {
	...
	const MenuItemsLists = menuItems
		.filter((item) => item.type === hackTools)
		.map((item: IRouterComponent) => {
			return (
				<Menu.Item
					key={item.key}
					icon={item.icon}
	...
	useHotkeys("ctrl+alt+1", () => {
		...
		setIndex("1");
		goTo(
			Tabs.filter((item) => item.type === HackToolsMode.web)[0].componentRoute
		);
	});
	useHotkeys("ctrl+alt+2", () => {
		...
		setIndex("1");
		goTo(
			Tabs.filter((item) => item.type === HackToolsMode.system)[0]
				.componentRoute
		);
	});
	useHotkeys("ctrl+alt+3", () => {
		...
		setIndex("1");
		goTo(
			Tabs.filter((item) => item.type === HackToolsMode.mobile)[0]
				.componentRoute
		);
	});
	useHotkeys("ctrl+alt+4", () => {
		...
		setIndex("1");
		goTo(
			Tabs.filter((item) => item.type === HackToolsMode.misc)[0].componentRoute
		);
	});
	...
	useHotkeys("ctrl+alt+c", () => {
		...
		goTo(
			Tabs.filter((item) => item.type === HackToolsMode.misc)[1].componentRoute
		);
	});
	/*--------------------------------*/
	...
	useEffect(() => {
		const currentComponent =
			Tabs.filter((tab) => tab.type === hackTools)[parseInt(index) - 1]
				.componentRoute || LayoutChoice;
			...
			<Layout style={{ minHeight: "100vh" }}>
				...
				<Layout className="site-layout" style={{ marginLeft: 80 }}>
					...
					<Footer style={{ textAlign: "center" }}>
						© HackTools - The all in one browser extension for offensive
						...
						<pre style={{ textAlign: "center" }}>
							<span>
								<Text keyboard>{keySymbol}</Text> + <Text keyboard>k</Text> to
								open the menu
							</span>
						</pre>
						<Button
						...
						/>
						<Button
							icon={<ArrowsAltOutlined style={{ margin: 5 }} />}
							onClick={() => windowMode()}
							type="link"
						>
							Pop-up mode
