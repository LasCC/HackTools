...
function QRGenerator () {
    ...
    const initialQRState = {
        ...
        email: { to: 'contact@me.com', subject: 'Hello', message: 'Hello from HackTools!' },
        tel: '+1234567890',
    ...
    useEffect( () => {
        if ( isDarkMode ) {
            message.info( 'For the best experience, use light mode for this component.' );
        }
    }, [ isDarkMode ] );
    ...
    return (
        <div style={{ padding: 8 }}>
            ...
            <Paragraph>
                During phishing exercises, you may want to send a link to the target that will redirect them to a malicious website. However, you may not want to send the link directly to the target, as they may be suspicious of the link and not click on it. Instead, you can send them a QR code that they can scan with their phone, which will redirect them to a controlled website.
                <br /><br />
                For more advanced phishing attacks (e.g. <Link href='https://owasp.org/www-community/attacks/Qrljacking' target='_blank'>QRLJacking</Link>), you can use the <Link href="https://breakdev.org/evilqr-phishing/" target="_blank">EvilQR</Link> tool to perform remote account takeover, through sign-in QR code phishing.
            </Paragraph>
            <Divider />
            <Row gutter={[ 16, 16 ]}>
                <Col span={10}>
                ...
                <Col span={14}>
                    <Form layout="vertical">
                        <Tabs defaultActiveKey="1" tabPosition="left" onChange={key => { setActiveTab( key ); resetQRCodeValue(); setDig( true ); }}>
                            ...
                            <TabPane tab={<Tooltip title="WIFI"><WifiOutlined /></Tooltip>} key="3">
                                <Space direction="vertical">
                                    <Text strong>WIFI</Text>
                                    <Text type="secondary">You can create a QR code that will connect the target to a WIFI network.</Text>
                                    <Form.Item label="Authentication">
                                        <Select
                                            defaultValue={formValues.wifi.auth}
                                            onChange={( value ) => handleNestedInputChange( 'wifi', 'auth', value )}
                                            options={[
                                                {
                                    ...
                                    <Form.Item label="SSID">
                                        <Input
                                            ...
                                            onChange={( e ) => handleNestedInputChange( 'wifi', 'ssid', e.target.value )}
                                        />
                                    </Form.Item>
                                    <Form.Item label="Password">
                                        <Input.Password
                                            ...
                                            onChange={( e ) => handleNestedInputChange( 'wifi', 'password', e.target.value )}
                                        />
                                    </Form.Item>
                                </Space>
                            </TabPane>
                            <TabPane tab={<Tooltip title="EMAIL"><MailOutlined /></Tooltip>} key="4">
                                <Space direction="vertical">
                                    <Text strong>Email</Text>
                                    <Text type="secondary">You can create a QR code that will send an email to the target.</Text>
                                    <Form.Item label="To">
                                        <Input
                                            placeholder="To: contact@hackme.com"
                                            value={formValues.email.to}
                                            onChange={( e ) => handleNestedInputChange( 'email', 'to', e.target.value )}
                                        />
                                    </Form.Item>
                            ...
                            <TabPane tab={<Tooltip title="TEL"><PhoneOutlined /></Tooltip>} key="5">
                                <Space direction="vertical">
                                    <Text strong>Telephone</Text>
                                    <Text type="secondary">You may need to add the country code before the number.</Text>
                            ...
                            <TabPane tab={<Tooltip title="SMS"><MessageOutlined /></Tooltip>} key="6">
                                <Space direction="vertical">
                            ...
                            <TabPane tab={<Tooltip title="Calendar Event"><CalendarOutlined /></Tooltip>} key="7">
                                <Space direction="vertical">
                                    <Text strong>Calendar Event</Text>
                                    <Text type="secondary">You can create a QR code that will add an event to the target's calendar.</Text>
                                    <Form.Item label="Title">
                                        <Input
                                            placeholder="Title"
                                            value={formValues.event.title}
                                            onChange={( e ) => handleNestedInputChange( 'event', 'title', e.target.value )}
                                        />
                                    </Form.Item>
                                    <Form.Item label="Description">
                                        <Input
                                            ...
                                            onChange={( e ) => handleNestedInputChange( 'event', 'description', e.target.value )}
                                        />
                                    </Form.Item>
                                    <Form.Item label="Location">
                                        <Input
                                            ...
                                            onChange={( e ) => handleNestedInputChange( 'event', 'location', e.target.value )}
                                        />
                                    </Form.Item>
                                    <Form.Item label="Date and Time">
                                        <DatePicker.RangePicker
                                            showTime
                                            value={[
                                                formValues.event.date[ 0 ] ? dayjs( formValues.event.date[ 0 ] ) : null,
                                                formValues.event.date[ 1 ] ? dayjs( formValues.event.date[ 1 ] ) : null
                                            ]}
                                            onChange={( dates ) => handleNestedInputChange( 'event', 'date', dates.map( d => d ? d.toISOString() : null ) )}
                                        />
                                    </Form.Item>
                                </Space>
                            </TabPane>
            ...
            <Row gutter={[ 0, 0 ]}>
                <Col span={24}>
                    <Space direction="vertical">
                        ...
                        <Text type="secondary">Select an icon to display in the center of the QR code.</Text>
                        <Space>
                ...
                <Drawer title="QRCode Configuration" placement="right" onClose={() => setOpen( false )} open={open}>
                    <Space direction="vertical">
                        <Text strong>Error Correction Level</Text>
                        <Text type="secondary">The error correction level is used to determine how much of the QR code can be damaged but still be readable.</Text>
                        ...
                        <Text type="secondary">⚠️ Setting the icon size too large may cause the QR code to be unreadable.</Text>
                        <Slider
                            min={10}
                            max={100}
                        ...
                        <ColorPicker
                            ...
                            showText={( color ) => <span>QRCode Color ({color.toHexString()})</span>}
                            onChange={( color ) => handleInputChange( 'color', color.toHexString() )}
                            presets={[ { label: 'Recommended', colors: [ '#000000', '#000000E0', '#000000A6', '#00000073', '#00000040', '#00000026', '#0000001A', '#00000012', '#0000000A', '#00000005', '#F5222D', '#FA8C16', '#FADB14', '#8BBB11', '#52C41A', '#13A8A8', '#1677FF', '#2F54EB', '#722ED1', '#EB2F96', '#F5222D4D', '#FA8C164D', '#FADB144D', '#8BBB114D', '#52C41A4D', '#13A8A84D', '#1677FF4D', '#2F54EB4D', '#722ED14D', '#EB2F964D', ], }, { label: 'Remove Background', colors: [ "#00000000" ] } ]}
                        />
                        <ColorPicker
                            allowClear
                            value={formValues.bgColor}
                            showText={( color ) => <span>QRCode Background Color ({color.toHexString()})</span>}
                            onChange={( color ) => handleInputChange( 'bgColor', color.toHexString() )}
                            presets={[ { label: 'Recommended', colors: [ '#000000', '#000000E0', '#000000A6', '#00000073', '#00000040', '#00000026', '#0000001A', '#00000012', '#0000000A', '#00000005', '#F5222D', '#FA8C16', '#FADB14', '#8BBB11', '#52C41A', '#13A8A8', '#1677FF', '#2F54EB', '#722ED1', '#EB2F96', '#F5222D4D', '#FA8C164D', '#FADB144D', '#8BBB114D', '#52C41A4D', '#13A8A84D', '#1677FF4D', '#2F54EB4D', '#722ED14D', '#EB2F964D', ], }, { label: 'Remove Background', colors: [ "#00000000" ] } ]}
                        />
                    </Space>
                </Drawer>
