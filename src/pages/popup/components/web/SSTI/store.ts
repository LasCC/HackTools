...
interface State {
    ...
    detectTemplate: ( snippet: string, userHints: string[] ) => DetectTemplateResult,
    setTemplateType: ( templateType: TemplateType ) => void,
    setGuessing: ( guessing: boolean ) => void,
    setPotentialTemplateEngine: ( potentialTemplateEngine: DetectTemplateResult | null ) => void,

}

const setPotentialTemplateEngine = ( set ) => ( potentialTemplateEngine: DetectTemplateResult | null ) => {
 ...
 */
function detectTemplate ( snippet: string, userHints: string[] ): DetectTemplateResult {
    const results = {};
    let totalMatches = 0;

    for ( const [ engine, { regex, languages } ] of Object.entries( templateEngines ) ) {
        if ( userHints.length === 0 || userHints.some( hint => languages.includes( hint ) ) ) {
    ...
    for ( const engine in results ) {
        ...
        const confidence = totalMatches > 0 ? Math.round( ( matchCount / totalMatches ) * 100 ) : 0;
        results[ engine ] = { confidence, languages };
    }

    return Object.entries( results )
        .filter( ( [ , { confidence } ]: [ string, { confidence: number } ] ) => confidence > 0 )
        .sort( ( [ , { confidence: a } ]: [ string, { confidence: number } ], [ , { confidence: b } ]: [ string, { confidence: number } ] ) => b - a )
